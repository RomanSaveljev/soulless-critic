...
function Review(parameters) {
  ...
  this.comments = function(file) {
    ...
    if (ret === undefined) {
      ret = [];
      var location = clone(REVIEW_COMMENTS.concat(file));
      location.push(ret);
      plantValue.apply(this.parameters, location);
    }
    return ret;
  ...
  this.url = function(url) {
    ...
    if (project === undefined) {
      ...
      var ret = extractValue.apply(this.parameters, GIT_PROJECT_CLONE.concat(project)) ||
        buildDefaultProjectUrl(this.parameters.configuration().git().server(), this.parameters.review().project());
      return ret && URI(ret);
    } else {
...
function CheckStyle(parameters) {
  ...
  this.enabled = function(enabled) {
    if (enabled === undefined) {
      return extractValue.apply(this.parameters, CHECKSTYLE_ENABLED);
    } else {
